...
export default async function DashboardPage(){
  ...
  const urgentLeads=leads.filter(item=>item.score>=70&&!['REJECTED','QUALIFIED'].includes(item.status));
  ...
  return <Dashboard firstName={session?.user?.name?.split(" ")[0]??"there"} role={founder?"founder":"salesperson"} metrics={{companyCount,urgentLeads:urgentLeads.length,pipelineValue,weightedValue,openDeals:openDeals.length,overdue:overdue.length,dueToday:dueToday.length,openInbox}} pipeline={pipeline} leads={urgentLeads.slice(0,5).map(item=>({id:item.id,title:item.title,score:item.score,company:item.company?.name??item.country??"Unlinked lead",owner:item.assignee?.name??"Unassigned"}))} tasks={tasks.slice(0,6).map(item=>({id:item.id,title:item.title,company:item.company?.name??"Internal",dueAt:item.dueAt,priority:item.priority,owner:item.assignee.name}))} meetings={meetings.map(item=>({id:item.id,title:item.title,startsAt:item.startsAt,company:item.company?.name??"Internal"}))} activities={activities.map(item=>({id:item.id,title:item.title,company:item.company?.name??"Workspace",occurredAt:item.occurredAt}))}/>;
}
