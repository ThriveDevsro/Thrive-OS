...
export default async function DashboardPage(){
  ...
  const [leads,opportunities,meetings,activities,companyCount]=await measureServerOperation("route:dashboard:data",()=>Promise.all([
  ...
  return <Dashboard firstName={session?.user?.name?.split(" ")[0]??"there"} role={session.user.role} metrics={{companyCount,attentionLeads:attentionLeads.length,pipelineValue,weightedValue,openDeals:openDeals.length,upcomingEvents:meetings.length}} pipeline={pipeline} leads={attentionLeads.slice(0,5).map(item=>({id:item.id,title:item.title,status:item.status,company:item.company?.name??item.country??"Unlinked lead",owner:item.assignee?.name??"Unassigned"}))} meetings={meetings.map(item=>({id:item.id,title:item.title,startsAt:item.startsAt,company:item.company?.name??"Internal"}))} activities={activities.map(item=>({id:item.id,title:item.title,company:item.company?.name??"Workspace",occurredAt:item.occurredAt}))}/>;
}
