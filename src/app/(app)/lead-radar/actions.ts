...
import { z } from "zod";
async function context(){const session=await auth();if(!session?.user)return null;const workspace=await prisma.workspace.findUnique({where:{slug:"thrive-dev"}});const user=await prisma.user.findFirst({where:{workspaceId:workspace?.id,email:session.user.email??undefined}});return workspace&&user?{workspace,user,founder:session.user.role==="founder"}:null}
